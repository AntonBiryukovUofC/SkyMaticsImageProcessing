'''
2nd step: preliminary clustering.
...
ins.setDef(1,10,3)
# If we want to extract features (takes a while), set to True
...
# Reject black images (up to 25%) BLACK should be picked from the dictionary !!!!!
NBlack = 1
inds = np.where(HistsMatrix[:,NBlack]/size < const)[0]
HistsMatrix = HistsMatrix[inds,:].squeeze()
list_im = [list_im[x] for x in inds]
...
print ' KMeans done'
# The new output folder with the clusters of images
